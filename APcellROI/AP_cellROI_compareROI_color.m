function AP_cellROI_compareROI_color
...
for curr_session = 1:length(tiff_files)
    ...
    for curr_roi_idx = 1:length(use_rois)
        ...
        rois(curr_roi_idx).im{curr_session} = nan(1+2*cell_y,1+2*cell_x,3,1);
        ...
        rois(curr_roi_idx).im{curr_session}(:,:,:,1) = repmat(mat2gray(cell_im_mean,[0 mean_max_px]),1,1,3);
        rois(curr_roi_idx).im{curr_session}(:,:,:,2) = repmat(mat2gray(cell_im_max,[0 max_max_px]),1,1,3);        
        ...
        if save_pcs > 0
            ...
            % reshape the PCs to square form
            ...
            % Colorize the PC scores and combine
            rn = abs(randn(1000,3));
            color_vectors_rand = bsxfun(@rdivide,rn,sqrt(sum(rn.^2,2)));
            warning off
            kidx = kmeans(color_vectors_rand,save_pcs,'Replicates',5);
            warning on
            [~,sort_idx] = sort(kidx);
            grpd_kidx = mat2cell(sort_idx,grpstats(sort_idx,kidx,'numel'));
            grp_rand = cellfun(@(x) x(randperm(length(x),1)),grpd_kidx);
            
            color_vectors_use = color_vectors_rand(grp_rand,:);
            
            cell_pc_score_color = arrayfun(@(x) ...
                mat2gray(repmat(cell_pc_score(:,:,x),1,1,3).* ...
                repmat(reshape(color_vectors_use(x,:),1,1,3), ...
                size(cell_pc_score,1),size(cell_pc_score,2),1)),1:save_pcs,'uni',false);
            
            cell_pc_score_color_combined = mat2gray(nansum(cat(4,cell_pc_score_color{:}),4));
            
            % Save PCs (3:end = PC scores)
            rois(curr_roi_idx).im{curr_session}(:,:,:,end+1) = cell_pc_score_color_combined;
        end
        ...
        if avg_active
            ...
            % generate color vector for each active portion (do this to
            % pseudomaximize distance between each vector)
            % NOTE: this isn't actually best solution because you want to
            % pull the colors to extremes, but I think this is a cool way
            % to evenly distribute points on a sphere so I'm leaving it
            % here
%             rn = abs(randn(1000,3));
%             color_vectors_rand = bsxfun(@rdivide,rn,sqrt(sum(rn.^2,2)));
%             warning off
%             kidx = kmeans(color_vectors_rand,length(activity_frames),'Replicates',5);
%             warning on
%             [~,sort_idx] = sort(kidx);
%             grpd_kidx = mat2cell(sort_idx,grpstats(sort_idx,kidx,'numel'));
%             grp_rand = cellfun(@(x) x(randperm(length(x),1)),grpd_kidx);
%             
%             color_vectors_use = mat2cell(color_vectors_rand(grp_rand,:),ones(length(activity_frames),1),3);
            
            % Assign each active portion a color vector (R,G, or B)
            rgb_vectors = [1 0 0;0 1 0;0 0 1];
            color_vectors_use = mat2cell(rgb_vectors(rem(0:length(activity_frames)-1,3)+1,:),ones(length(activity_frames),1),3);
            
            activity_color = cellfun(@(frames,col) repmat(cell_im(:,frames),1,1,3).* ...
                repmat(reshape(col,1,1,3),size(cell_im(:,frames),1),length(frames),1), ...
                activity_frames,color_vectors_use','uni',false);
            
            activity_color_cat = cat(4,activity_color{:});
            activity_color_cat_mean = permute(nanmean(activity_color_cat,4),[1 3 2]);
            activity_color_cat_mean_reshape = reshape(activity_color_cat_mean,sum(cell_y_use),sum(cell_x_use),3,[]);
            
            % Normalize the caxis based on the ROI
            curr_mask = poly2mask(rois(curr_roi_idx).polygon{curr_session}(:,1), ...
                rois(curr_roi_idx).polygon{curr_session}(:,2), ...
                sum(cell_y_use),sum(cell_x_use));
            max_px = max(activity_color_cat_mean_reshape(repmat(curr_mask,1,1,3, ...
                size(activity_color_cat_mean_reshape,4)))) * 1.5;
            
            curr_act_im = mat2gray(activity_color_cat_mean_reshape,[0 max_px]);
                        
            total_frames = pre_frames + 2*updown_frames + post_frames;
            
            if ~isempty(curr_act_im)
                rois(curr_roi_idx).im{curr_session}(cell_y_use,cell_x_use,:,end+1:end+size(curr_act_im,4)) = curr_act_im;
            else
...
for curr_session = 1:length(rois(1).im)
    handles.im(curr_session) = ...
        image(rois(1).im{curr_session}(:,:,:,1),'parent',handles.im_axes(curr_session));
...
for curr_session = 1:length(gui_data.rois(curr_roi).im);
    num_ims = size(gui_data.rois(curr_roi).im{curr_session},4);
    if new_im > num_ims
        new_im = num_ims;
    elseif new_im < 1
        new_im = 1;
...
if save_rois
    ...
    for curr_session = 1:length(gui_data.handles.roi_files)
        ...
        curr_savefile = [save_path filesep gui_data.handles.roi_files{curr_session}(1:end-4) '_checked.roi'];
