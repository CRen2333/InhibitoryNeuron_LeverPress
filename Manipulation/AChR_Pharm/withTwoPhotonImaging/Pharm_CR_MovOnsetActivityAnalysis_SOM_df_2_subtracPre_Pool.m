...
switch SessionType
    case 'Nai_Ant'
        ...
        Drug = 'Ant';
    case 'Exp_Ago'
       Animals = {'WL_3547273-LR','WL_3547272-O','WL_3547272-L','WL_3526578-O','WL_3526578-R','CR_3672035-L','CR_3672035-R','CR_3702224-L','CR_3702224-O','CR_3702224-R','CR_3702226-O'};           
end
...
for curr_animal = 1:length(Animals)
    ...
    switch SessionType
        case 'Exp_Ago'
            if ismember(Animal,{'CR_3658844-L','CR_3658844-R','CR_3658845-R','CR_3672031-O','CR_3633193-L','CR_3633193-R'})
    ...
    LeverTrace_Path = fullfile('Z:\People\Chi\TwoP_IN',IN,Animal,'Pharm',SessionType_2,'LeverTrace');
    Dates = dir(LeverTrace_Path);
    Dates = {Dates.name};
    Dates = sort(Dates(cellfun(@(x) contains(x, '17')||contains(x, '18')||contains(x, '19'), Dates)))';
    % load imaging info
    load(fullfile('Z:\People\Chi\TwoP_IN',IN,Animal,'Pharm',SessionType_2,'df_f',[Animal '_ImagingInfo.mat']),'Imaging_Fields','-mat');
    % load mov-onset frame index
    load(fullfile('Z:\People\Chi\TwoP_IN',IN,Animal,'Pharm',SessionType_2,'MovAnalysis',[Animal '_CuedRewardedMov_FrameIndex.mat']),'CuedRewardedFrameIndex','CuedRewardedFrameIndex_z1','CuedRewardedFrameIndex_z2','-mat');
    for curr_field = 1:length(Imaging_Fields)
        ...
        for curr_date = 1:length(Field_Dates)
            ...
            df_f_Path = fullfile('Z:\People\Chi\TwoP_IN',IN,Animal,'Pharm',SessionType_2,'df_f',Field);
...
for curr_animal = 1:length(Animals)
    ...
    switch SessionType
        case 'Exp_Ago'
            if ismember(Animal,{'WL_3547273-LR','WL_3547272-O','WL_3547272-L','WL_3526578-O','WL_3526578-R'})
                SessionType_2 = 'Exp_Ago';
            else
                SessionType_2 = 'Exp_Ago_Ant';
            end
        otherwise
    ...
    for curr_field = 1:size(df_f_MovOnset_Alinged,1)
        ...
        if strcmp(SessionType,'Nai_Ant') && strcmp(Imaging_Fields{1}.SessionType{1},'Nai_Ant')
...
load(['Z:\People\Chi\TwoP_IN\' IN filesep 'Pharm' filesep SessionType '\Figures_SubPre\SOM_ActivityAnalysis_SubPre.mat'],'Animals','Post_MovOnset_Aligend_df_sub_Pharm','-mat');
...
% color_value = [176,136,216;143,96,190;96,45,149;73,27,121]./255;
% color_line = [116,66,166]./255;
Pharms = {'Ctrl',Drug};
...
for ii = 1:2
%     plot(ii,temp_1(ii),'Marker','.','MarkerSize',20,'color',color_line);
...
% stairs(edges(1:end-1)+0.05,temp_n,'color',color_value(4,:),'linewidth',1);
...
for ii = 1:2
    ...
    temp = temp(I,:);
%     imagesc(temp,[-2.5 2.5]); colormap(color_map);
    imagesc(temp,[-0.8 2.5]); colormap(mycmap);
...
for ii = 1:2
    subplot(1,2,ii);
    imagesc(all_df_f_MovOnset_Alinged_EachROI_MeanAXTrial_Pharm{ii}(I_ctrl,:),[-0.5 2.0]);
...
xlim([0 0.4]);ylim([1 length(Animal_tag)]);set(gca,'ydir','reverse');
...
%         line([ii-0.5,ii+0.5],[0.28 0.28],'color','k','linewidth',1);
        line([ii-0.5,ii+0.5],[0.11 0.11],'color','k','linewidth',1);
    end
end
...
xticks([0:20:40]);% xticklabels({'inc.','dec.','no'});
yticks([0:20:40]);    
axis square;
...
% end
% xlim([0.7 3.3]);xticks([1:3]);xticklabels({'inc.','dec.','no'});
% ylabel('Fraction')
% title(Pharms{ii});
% axis square;
...
saveas(gcf, [FigurePath filesep 'comb_Pool_' IN '_Fraction_MvmActSup.pdf'],'pdf'); pause(1);        
...
ii = 2;
cellnum = length(All_movement_activated_cells_onset{ii});
plot(sort(All_movement_activated_cells_onset{ii},'ascend'),[1:cellnum]/cellnum,'color',color_value(4,:),'linewidth',1);
line([0 0],ylim,'color','k','linestyle',':');
xlim([-0.5 2]);
...
ii = 2;
cellnum = length(All_movement_suppressed_cells_onset{ii});
plot(sort(All_movement_suppressed_cells_onset{ii},'ascend'),[1:cellnum]/cellnum,'color',color_value(4,:),'linewidth',1);
line([0 0],ylim,'color','k','linestyle',':');
xlim([-0.5 2]);
xlabel('Activity onset (s)'); ylabel('cum. prob.');
...
set(h(2),'EdgeColor','none','FaceColor',color_value(4,:),'FaceAlpha',0.3);
plot(temp_1,'color',color_value(4,:),'linewidth',2);
line([8,8],ylim,'color','k','linestyle',':','linewidth',1); ylim([-0.05, 0.45]);
xlim([1,36]); xticks([8,22,36]); xticklabels({'0','1','2'}); xlabel('Time (sec)');
...
set(h(2),'EdgeColor','none','FaceColor',color_value(4,:),'FaceAlpha',0.3);
plot(temp_1,'color',color_value(4,:),'linewidth',2);
line([8,8],ylim,'color','k','linestyle',':','linewidth',1); ylim([-0.4, 0.05]);
xlim([1,36]); xticks([8,22,36]); xticklabels({'0','1','2'}); xlabel('Time (sec)');
