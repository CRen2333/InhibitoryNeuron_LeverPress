function [Electrode_CaEvents_Onset, Electrode_CaEvents_Offset, Electrode_CaEvents] = CR_GetCaEvents_WF(Electrode_df_f,threshold,framerate,smooth_window_sec,Plotting)
% Plotting = true or false;
%% Start
% Loess smooth over 1 sec
Electrode_Smooth_loess = smooth(Electrode_df_f,framerate*smooth_window_sec,'loess');
noise_df_f = abs(Electrode_df_f-Electrode_Smooth_loess);
noise_cutoff = std(Electrode_df_f-Electrode_Smooth_loess)*threshold; % noise cutoff
...
Electrode_Smooth_active_switch = diff([0;Electrode_Smooth_active;0]);
% Get rough onset: when the velocity above threshold_active
temp_onset = find(Electrode_Smooth_active_switch==1);
temp_offset = find(Electrode_Smooth_active_switch==-1);
% ignore if too short
active_interval = temp_offset-temp_onset;
temp_onset(active_interval<5) = [];
temp_offset(active_interval<5) = [];

if Plotting
    hold on
    for ii = 1:length(temp_onset)
        hold on; line([temp_onset(ii),temp_onset(ii)],[-1, 1],'color',[0.5 0.5 0.5]);
    end
end
...
if Plotting
    ...
    for ii = 1:length(temp_offset)
        hold on; line([temp_offset(ii),temp_offset(ii)],[-1, 1],'color',[0.5 0.5 1]);
    end
end
% Fill the gap:
...
if Plotting
    ...
    hold on; plot(temp.*Electrode_df_f,'m');
    for ii = 1:length(temp_offset)
        hold on; line([temp_onset(ii),temp_onset(ii)],[-1, 1],'color',[0.3 0.3 0.3]);
        hold on; line([temp_offset(ii),temp_offset(ii)],[-1, 1],'color',[0.3 0.3 1]);
    end
end
...
%     Electrode_CaEvents(temp_onset(ii):temp_offset(ii)) = Electrode_df_f(temp_offset(ii))-baseline_df_f(ii);
    Electrode_CaEvents(temp_onset(ii):temp_offset(ii)) = 1;
end
if Plotting
