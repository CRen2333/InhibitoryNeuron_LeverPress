...
for curr_animal = 1:length(Animals)
    ...
    %% Clean
    clearvars -except Initial Animals IN curr_animal Gap MovDur Leeway Stage;
    clc;
    ...
    General_path = ['Z:\People\Chi\TwoP_IN\' IN filesep Animal filesep Stage];
    cd(General_path);
    mkdir('MovAnalysis');
    ...
    for curr_day = 1:length(Dates)
        ...
        if strcmp(Animal, 'CR_4259762-LL') && strcmp(Date,'210518')
            disp('Behavior computer crashed, corrupted Bpod file');
            continue
        end
        clear Frame_times Lever_active Lever_active_switch Lever_force_resample Licking_active Licking_bout Likcing_bout switch TrialInfo_xsg 
        ...
        if ~exist([Xsg_path filesep Date filesep Animal '_' Date '_EphusTraces.mat'])
            warning(['No Ephus extraction file saved for ' Animal ' on ' Date '.']);
            continue
        end
        ...
        clear TrialInfo_Bpod CuedMov
        for curr_session = 1:length(Curr_Bpod_filname)
            if strcmp(Animal,'CR_4017386-L') && str2num(Date) < 200525
                Lever_force_resample{curr_session} = -Lever_force_resample{curr_session};
            end
            warning off
...
%             clear TrialInfo_Bpod
            ...
            CuedMov.ActiveTrial{curr_session} = [];
            ...
            for curr_trial = 1:TrialNum_Total
                ...
                StiTime.CueOn{curr_day}{curr_xsg}(curr_trial,1) = Cue_Start_xsg;
                StiTime.CueOff{curr_day}{curr_xsg}(curr_trial,1) = Cue_End_xsg;
                StiTime.RewardOn{curr_day}{curr_xsg}(curr_trial,1) = Reward_xsg;
                
                if sum(Lever_active{curr_session}(Cue_Start_xsg:End_xsg))~= 0
                    MvmTrial(curr_trial) = 1;
                else
                    MvmTrial(curr_trial) = 0;
                end
                Temp_MovOnset_info = [];
                ...
                if ~isempty(MovOnset_curr_trial)
                    ...
                    for curr_movonset = 1:length(MovOnset_curr_trial)
                        % ignore if exceeds
                        if MovOnset_curr_trial(curr_movonset)+Sample_range_resample(1) <0 || MovOnset_curr_trial(curr_movonset)+Sample_range_resample(end) >TimeUp
                            continue
                        end
            ...
            CuedMov.ActiveTrial{curr_session} = MvmTrial;
        ...
        if ~isempty(CuedMov.LeverTrace_First)
        ...
        for curr_session = 1:length(Frame_times)
            if isempty(CuedMov.Cued_MovOnset_Info{curr_session})
                MovActiveFrame{curr_day}{curr_session} = [];
                CuedRewardedFrameIndex{curr_day}{curr_session} = [];
                CuedRewardedFrameIndex_1{curr_day}{curr_session} = [];
                CuedRewardedFrameIndex_2{curr_day}{curr_session} = [];                
                continue
            end
    ...
    % Save
    cd(['Z:\People\Chi\TwoP_IN\' IN filesep Animal filesep Stage filesep 'MovAnalysis']);
