function [results, warp, warpedImage, rho_final] = ecc_old(image, template, levels, noi, transform, delta_p_init, mask, margin_ratio)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for nol=2:levels
    ...
    MASK{nol} = imresize(MASK{nol-1},.5,'nearest');
end
...
if strcmp(transform,'translation')
    ...
    if nargin==5||isempty(delta_p_init);
        warp=zeros(2,1);
    else
...
for nol=levels:-1:1
    ...
    if(~exist('margin_ratio','var')||isempty(margin_ratio))
        margin_ratio = 0;
    end
    margin=floor(m0*margin_ratio/(2^(nol-1)));
...
%         end
        ...
        ones_map = mask(ny,nx) & spatial_interp(ones(size(im)), warp, 'nearest', transform, nx, ny); %inverse (backward) warping
        % Edited 6/7/17 by RH. Originally, this line was ones_map = mask(nx,ny) & spatial_interp(ones(size(im)), warp, 'nearest', transform, nx, ny); %inverse (backward) warping
