function applywarp_Npatches(fns_tiff_name,fns_summary_name,foldername,local2savefoldername,local_filename,ch,n_ch,warp_cell,transform,rl_RemovingPix,nonzero_row,nonzero_column,qN_x,qN_y,overlap_npix,block_size)
    % check if warped, summary file has been changed by adding a 'warp'
    % variable
    temp_summary = load(fullfile(foldername,fns_summary_name));
...
%     if ~ispc
        img_filename = fullfile(foldername, fns_tiff_name);
%     else
...
%     end
    ...
    stack = stack(:,1+rl_RemovingPix:end-rl_RemovingPix,:); % remove n pixels from left and right edges.
    ...
    for i3 = 1:size(stack,3)
        ...
        for i1 = 1:block_size
            for i2 = 1:block_size
                out_temp{i1,i2} = spatial_interp(double(stack(qN_y{i1,i2},qN_x{i1,i2},i3)), warp_cell{i1,i2}, 'cubic', transform, 1:length(qN_x{i1,i2}), 1:length(qN_y{i1,i2}));
...
%         disp(['save temp file:', fns_tiff_name(1:end-4),'_warped.tif']);
        write_tiff(fullfile(local2savefoldername, [fns_tiff_name(1:end-4),'_warped.tif']),...
            int16(stack_warp),info);
        %disp('replace original tiff')
        %[~,msg] = copyfile(fullfile(foldername, [fns_tiff_name(1:end-4),'_warped.tif']), ...
        %    fullfile(foldername, 'motioncorrected_tiff',fns_tiff_name));
        %disp(msg);
        %delete
        %delete(fullfile(foldername, [fns_tiff_name(1:end-4),'_warped.tif']));
        %replace summary file
%         disp('replace summry file')
        save(fullfile(local2savefoldername,[fns_summary_name(1:end-4) '_warped.mat']), '-struct','temp_summary');
        clear temp_summary
    else
...
%         disp('replace original tiff')
        [~,msg] = copyfile(img_filename, ...
            fullfile(local2savefoldername,[fns_tiff_name(1:end-4) '_warped.tif']));
%         disp(msg);
%         disp('replace summry file')
        save(fullfile(local2savefoldername,[fns_summary_name(1:end-4) '_warped.mat']), '-struct','temp_summary');
        clear temp_summary
