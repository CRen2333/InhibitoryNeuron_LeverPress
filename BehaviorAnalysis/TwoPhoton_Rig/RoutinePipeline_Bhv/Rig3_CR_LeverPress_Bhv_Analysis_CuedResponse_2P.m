...
for curr_animal = 1:length(Animals)
    ...
    %% Clean
    clearvars -except Initial Animals IN curr_animal Gap MovDur Leeway PreCue;
    clc;
    ...
    for curr_day = 1:length(Dates)
        ...
        for curr_session = 1:length(Curr_Bpod_filname)
            warning off
            data = load([Bpod_path filesep Curr_Bpod_filname{curr_session}]);
            warning on
        
            % Get general trial information that will be used for all xsg files
            % Extract trial information from Bpod
            TrialInfo_Bpod = data.saved_history.ProtocolsSection_parsed_events;
            CatchTrials = cell2mat(data.saved_history.CatchSection_catch_trial);
            CatchTrials = CatchTrials(1:length(TrialInfo_Bpod));
            clear data;
            % Total trial num
            TrialNum_Total = min(length(TrialInfo_Bpod),size(TrialInfo_xsg{curr_session},1));
        
            % Get rewarded trials
            Rewarded_trial = find(cellfun(@(x) ~isempty(x.states.reward), TrialInfo_Bpod));
            ...
            for curr_trial = 1:TrialNum_Total
                curr_trial_index = TrialInfo_xsg{curr_session}(curr_trial,2);
                if curr_trial_index > TrialNum_Total
                    continue
                end
                % Cue time index in lever trace
                Bitcode_dispatcher = TrialInfo_Bpod{curr_trial_index,1}.states.bitcode(1);
                % Cue time
                Cue_Start_dispatcher = TrialInfo_Bpod{curr_trial_index,1}.states.cue(1);
                Cue_End_dispatcher = TrialInfo_Bpod{curr_trial_index,1}.states.cue(2);
                Cue_Start_xsg = Cue_Start_dispatcher - Bitcode_dispatcher + TrialInfo_xsg{curr_session}(curr_trial,1);
                Cue_End_xsg = Cue_End_dispatcher - Bitcode_dispatcher + TrialInfo_xsg{curr_session}(curr_trial,1);
                % Reward time
                if ismember(curr_trial_index, Rewarded_trial)
                    Reward_dispatcher = TrialInfo_Bpod{curr_trial_index,1}.states.reward(1);
                    Reward_xsg = Reward_dispatcher - Bitcode_dispatcher + TrialInfo_xsg{curr_session}(curr_trial,1);
                else
                    Reward_xsg = nan;
                end
                Cue_Start_xsg = round(Cue_Start_xsg*1000);
                Cue_End_xsg = round(Cue_End_xsg*1000);
                Reward_xsg = round(Reward_xsg*1000);               
                ...
                if ~isempty(MovOnset_curr_trial)
                    ...
                    if ~isnan(Reward_xsg)
                        ...
                        if ~isempty(Temp_Rewarded_MovOnset)
                            if Temp_Rewarded_MovOnset >= Cue_Start_xsg && sum(Lever_active{curr_session}((round(Cue_Start_xsg)-PreCue):round(Cue_Start_xsg)))==0 && (Temp_Rewarded_MovOnset+Sample_range_resample(end))/1000 <= TimeUp % Collect trace
                    ...
                    for curr_movonset = 1:length(MovOnset_curr_trial)
                        ...
                        Temp_MovOnset_info(curr_movonset,10) = CatchTrials(curr_trial_index);
...
%             CuedMov.Bpod{curr_session} = TrialInfo_Bpod{curr_session};
        ...
        if isempty(CuedMov.LeverTrace_All)
            ...
            if isempty(CuedMov.LeverTrace_Reward)
                CuedMov.LeverTrace_Reward_downsample = [];
            else
        ...
        for curr_session = 1:length(Frame_times)
            ...
            if ~isempty(Frame_times{curr_session})
                for curr_trial = 1:length(CuedRewardedMovOnset)
                    ...
                    CuedRewardedFrameIndex{curr_day}{curr_session}(:,curr_trial) = [I_onset-15:I_onset+60];
                    ...
                    CuedRewardedFrameIndex_z1{curr_day}{curr_session}(:,curr_trial) = [I_onset_z1-7:I_onset_z1+30];
                    ...
                    CuedRewardedFrameIndex_z2{curr_day}{curr_session}(:,curr_trial) = [I_onset_z2-7:I_onset_z2+30];
                end
    ...
    % Save
    cd(['Z:\People\Chi\TwoP_IN\' IN filesep Animal filesep 'MovAnalysis']);
