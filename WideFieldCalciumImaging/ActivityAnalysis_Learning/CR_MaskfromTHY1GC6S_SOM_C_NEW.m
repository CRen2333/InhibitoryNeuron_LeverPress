...
for curr_animal = 1:length(Animals)
    ...
    clear df_f_ROI_sub_1st_trial_arranged df_f_ROI_trial_arranged Final_Mask
end   
...
for curr_animal = 1:length(animalID)
    ...
    for curr_ROI = 1:length(ROI)
        ...
        for curr_session = 1:11
            hold on; plot(Mean_Cued_subtract_ROI_df_f_arranged{curr_ROI}{curr_session}(curr_animal,:),'color',colorvalue(curr_session*5,:));
            xlim([1 76]);
        end
        title(ROI{curr_ROI});
    end
...
for curr_ROI = 1:length(ROI)
    ...
    xlim([1 76]); ylim([-0.008, 0.03]); ylabel('df/f'); xlabel('Time (sec)');
...
xlim([1 76]); ylim([-0.007, 0.029]); ylabel('df/f'); xlabel('Time from mvm. onset (sec)');
set(gca,'XTick',[16,46,76],'XTickLabel',{'0','1','2'});
