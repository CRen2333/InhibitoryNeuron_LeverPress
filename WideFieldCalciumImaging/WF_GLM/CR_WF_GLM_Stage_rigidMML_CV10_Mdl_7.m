...
% use rigidMML (Musall, NN, 2019) to make it faster
...
%             temp = ~(abs(diag(fullQRR)) > max(size(predictors)) * eps(fullQRR(1)));
            ...
            for cv_ii = 1:cv_num
                ...
                [MSE_train(cv_ii,:),MSE_test(cv_ii,:),VarExp_train(cv_ii,:),VarExp_test(cv_ii,:),Corr_train(cv_ii,:),Corr_test(cv_ii,:)] = ...
                    CR_EvaluatePostrigidMML(df_f, temp_fit_trace, training_index);
        ...
        file_name = [save_folder filesep IN '_' Initial '_' Animal '_df_f_all_pseudoICA_GLM_stage_rigidMML_CV10.mat'];
...
saveas(gcf,['VIP_CVRsquare_rigidMML.fig'],'fig'); pause(1);
saveas(gcf,['VIP_CVRsquare_rigidMML.png'],'png');
...
saveas(gcf,['SOM_CVRsquare_rigidMML.fig'],'fig'); pause(1);
saveas(gcf,['SOM_CVRsquare_rigidMML.png'],'png');
...
saveas(gcf,['PV_CVRsquare_rigidMML.fig'],'fig'); pause(1);
saveas(gcf,['PV_CVRsquare_rigidMML.png'],'png');
close all
...
saveas(gcf,['CorticalMean_CVRsquare_rigidMML.fig'],'fig'); pause(1);
saveas(gcf,['CorticalMean_CVRsquare_rigidMML.png'],'png');
close all