%% Calculate df/f for each pixel in wide-field calcium images
% Based on Hiroshi's coding and Takaki's suggestion
...
if isempty(Check) % df_f hasn't been started yet
    ...
    for block = 1:totalBlock
        ...
        save([Initial '_' Date '_' Animal '_baseline_all'],'baseline_all','-v7.3');
        save([Initial '_' Date '_' Animal '_df_all'],'df_all','-v7.3');
        save([Initial '_' Date '_' Animal '_df_f_all'],'df_f_all','-v7.3');
    ...
    load([Initial '_' Date '_' Animal '_df_f_all'],'df_f_all');
    ...
    if length(df_f_all) == totalBlock % All blocks are finished
        ...
        if ~isempty(Check_baseline) % if beseline_all exists
            load([Initial '_' Date '_' Animal '_baseline_all'],'baseline_all');
        end
        if ~isempty(Check_df) % if df_all exists
            load([Initial '_' Date '_' Animal '_df_all'],'df_all');
        end
        % Continue
