%% Get roi traces & df/f, for S1HL and PPC
clear all;
close all;
clc;
...
% Assume it's all FOV1, seperate later manually
...
for curr_animal = 1:length(Animals)
    ...
    for curr_field = 1:length(Fields)
        ...
        cd([General_path filesep 'df_f' filesep Field]);
        ...
        for curr_day = 1:length(Dates)
            ...
            for curr_session = 1:length(roi_trace)
                ...
                figure; set(gcf,'position',[0,0,800,400]); hold on;
                ...
                close(gcf);
                figure; set(gcf,'position',[50,50,800,400]); hold on;
                for curr_roi = 1:size(roi_for_qualitycheck,1)
...
%% Reject bad rois
clear
load('CR_4412583-LR_210930_ROI_Traces.mat');
Rejected_ROIs = [2,4];
...
save('CR_4412583-LR_210930_ROI_Traces.mat','roi_trace_df_2','Rejected_ROIs','-append');
clear
