function [roi_trace_df, roi_trace_baseline] = CR_AP_baselineEstimation_Axon(roi_trace_long,framerate,isbg,baseline_smooth_window_min)
% [roi_trace_df roi_trace_baseline] = AP_baselineEstimation(roi_trace_long,framerate, isbg)
% roi_trace_df: the df/f normalized trace
% roi_trace_baseline (optional): the estimated baseline of the trace
% 
% Modification
% 2016-5-6 HL change the criteria for NaN a cell adjustable
% usage: 0.99 for background; 0.95 for cell
% 2016-5-22 HL change criteria back to 95%
% make the first input as background baseline estimate flag
% if estimating baseline, keep result, do NOT nan cells
%%
disp('Baseline correcting traces (%):')
fprintf('%2d',0);
if nargin < 3
    isbg = 0; %default for roi trace    
end
baseline_est_runs = 2; % default 2
% HL add in toss_crit
toss_crit = 1; %default 95 percentage active is suspective for cells
...
for curr_cell = 1:num_rois;
    ...
    for baseline_est = 1:baseline_est_runs;
        ...
        baseline_smooth_min = baseline_smooth_window_min;
        ...
        smooth_reliable = ceil(baseline_smooth_frames/2/baseline_smooth_window_min);
        ...
        activity_surround_sec = 1; % default 5
...
%         curr_trace = inactive_trace;
    ...
    % HL 2016-5-6 change to 99%
%     length(curr_trace)/length(roi_trace_long(curr_cell,:))
    if length(curr_trace) <= (1-toss_crit)*length(roi_trace_long(curr_cell,:)) && ~isbg % not background estimate
        roi_trace_df(curr_cell,:) = nan(1,size(roi_trace_df,2));
        %[HL] report this cell is active 95%
%         disp('curr trace < 5% of total trace')
        continue
    end
...
%      end
    ...
    inactive_baseline_full = ...
        interp1(find(~active_indx_surround), ...
        inactive_baseline_smooth(~active_indx_surround), ...
        [1:length(inactive_baseline_smooth)]);% [HL]temporal relationship changed?
    ...
    % if (incredibly rare - and the cells are probably filled anyway)
    % active 95% of the time, skip the cell
%     sum(active_indx_surround)/length(active_indx_surround)

    if sum(active_indx_surround) >= toss_crit*length(active_indx_surround) && ~isbg
        roi_trace_df(curr_cell,:) = nan(1,size(roi_trace_df,2));
        continue
    end
    if isempty(inactive_df_smooth(~active_indx_surround))||...
            length(inactive_df_smooth(~active_indx_surround))<2
        disp(curr_cell)
        error('no inactive period left')
    end
