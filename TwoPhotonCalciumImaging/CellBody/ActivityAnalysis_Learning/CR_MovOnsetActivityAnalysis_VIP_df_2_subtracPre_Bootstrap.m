...
for curr_animal = 4%:length(Animals)
    ...
    if strcmp(Animal,'KP_3463808_1')
        for curr_roi = 1:size(df_f_MovOnset_Alinged{1,10},1)
            df_f_MovOnset_Alinged{1,10}{curr_roi,1} = df_f_MovOnset_Alinged{1,10}{curr_roi,1}([1:2:72],:);
        end
        for curr_roi = 1:size(df_f_MovOnset_Alinged{1,11},1)
            df_f_MovOnset_Alinged{1,11}{curr_roi,1} = df_f_MovOnset_Alinged{1,11}{curr_roi,1}([2:2:72],:);
        end
    end
