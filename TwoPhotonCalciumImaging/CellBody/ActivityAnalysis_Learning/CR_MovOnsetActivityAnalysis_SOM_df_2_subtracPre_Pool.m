...
for curr_animal = 1:length(Animals)
    ...
    for curr_field = 1:size(df_f_MovOnset_Alinged,1)
        ...
        % Get 4 stages first, get mean then combine fields
...
IN = 'SOM';
load(['Z:\People\Chi\TwoP_IN\' IN '\Figures_SubPre\' IN '_ActivityAnalysis_SubPre.mat'],'Animals','Post_MovOnset_Aligend_df_stage','nan_cells','-mat');
...
% Distribution
figure; set(gcf,'color','w','position',[400,0,600,600]); hold on;
for ii = 1:4
...
for ii = 4
    ...
    histogram(Delta_df_against_N(:,ii),edges,'Normalization','probability','edgecolor','none','facecolor',color_value(ii,:));
    ylim([0 0.4]); xlim([edges(1) edges(end)]);
    line([0 0],ylim,'color','k','linestyle',':');
    xlabel('delta df/f'); ylabel('Prob.'); axis square;
...
% Temporal traces
load(['Z:\People\Chi\TwoP_IN\' IN '\Figures_SubPre\' IN '_ActivityAnalysis_SubPre.mat'],'df_f_MovOnset_Alinged_EachROI_MeanAXTrial_stages','-mat');
for ii = 1:4
...
%     imagesc(all_df_f_MovOnset_Alinged_EachROI_MeanAXTrial_stages{ii}(I_naive,:),[-0.5 1.7]);
...
Animals_color = Animals_color(10:6:64,:);
for ii = 1:length(Animals)
...
load('Z:\People\Chi\TwoP_IN\SOM\Figures_SubPre\SOM_ActivityAnalysis_SubPre.mat', 'Delta_df_against_N_CellTag')
...
figure; hold on; set(gcf,'color','w','position',[500 200 800 200]);
for ii = 1:4
    subplot(1,4,ii);
...
load('Z:\People\Chi\TwoP_IN\SOM\Figures_SubPre\SOM_ActivityAnalysis_SubPre.mat','movement_activated_cells_onset_stage','movement_suppressed_cells_onset_stage','-mat');
for ii = 1:4
...
saveas(gcf, [FigurePath filesep 'combo_Pool_' IN '_fraction_mvm_in_celltag_inc.fig']); pause(1);
saveas(gcf, [FigurePath filesep 'combo_Pool_' IN '_fraction_mvm_in_celltag_inc.png']); pause(1);
saveas(gcf, [FigurePath filesep 'combo_Pool_' IN '_fraction_mvm_in_celltag_inc.pdf']); pause(1);   
