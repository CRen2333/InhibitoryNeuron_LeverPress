function varargout = GUI_CR_WF_PupilCheck_Baseline(varargin)
% GUI_CR_WF_PUPILCHECK_BASELINE MATLAB code for GUI_CR_WF_PupilCheck_Baseline.fig
%      GUI_CR_WF_PUPILCHECK_BASELINE, by itself, creates a new GUI_CR_WF_PUPILCHECK_BASELINE or raises the existing
%      singleton*.
%
%      H = GUI_CR_WF_PUPILCHECK_BASELINE returns the handle to a new GUI_CR_WF_PUPILCHECK_BASELINE or the handle to
%      the existing singleton*.
%
%      GUI_CR_WF_PUPILCHECK_BASELINE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_CR_WF_PUPILCHECK_BASELINE.M with the given input arguments.
%
%      GUI_CR_WF_PUPILCHECK_BASELINE('Property','Value',...) creates a new GUI_CR_WF_PUPILCHECK_BASELINE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_CR_WF_PupilCheck_Baseline_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_CR_WF_PupilCheck_Baseline_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_CR_WF_PupilCheck_Baseline

% Last Modified by GUIDE v2.5 06-Nov-2017 16:51:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_CR_WF_PupilCheck_Baseline_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_CR_WF_PupilCheck_Baseline_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_CR_WF_PupilCheck_Baseline is made visible.
function GUI_CR_WF_PupilCheck_Baseline_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_CR_WF_PupilCheck_Baseline (see VARARGIN)

% Choose default command line output for GUI_CR_WF_PupilCheck_Baseline
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_CR_WF_PupilCheck_Baseline wait for user response (see UIRESUME)
% uiwait(handles.fig);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_CR_WF_PupilCheck_Baseline_OutputFcn(hObject, eventdata, handles) 
...
handles.Filename = [Initial '_' Date '_' Animal 'Baseline_Fitting'];
...
handles.TotalFrameSelected = nan(1,size(Eye_ROI_filtered,3));
...
imagesc(handles.Image(:,:,handles.FrameNum)); colormap(gray); axis equal;
hold on;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.job_state = 'Finish';
assignin('base','job_state',handles.job_state);
guidata(hObject,handles);
...
elseif handles.TotalFrameSelected(handles.FrameNum) == 1
    ...
    hold on;
    handles.h1 = ellipse(handles.bestFits{handles.FrameNum}(1,3),handles.bestFits{handles.FrameNum}(1,4),handles.bestFits{handles.FrameNum}(1,5)*pi/180,handles.bestFits{handles.FrameNum}(1,1),handles.bestFits{handles.FrameNum}(1,2));
    set(handles.h1,'color',[0.75 0.93 0.1]);
elseif handles.TotalFrameSelected(handles.FrameNum) == 2
    set(handles.checkbox_Fit_1,'Value',0);
    set(handles.checkbox_Fit_2,'Value',1);
    set(handles.checkbox_Fit_3,'Value',0);
