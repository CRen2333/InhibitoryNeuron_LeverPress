...
if ~all(cellfun(@(x) any(cellfun(@(y) any(strfind(x(1:end-4),y(1:end-4))),tiff_files)),roi_files));
...
for curr_session = 1:length(tiff_files)
    ...
    for curr_roi_idx = 1:length(use_rois)
        ...
        mean_max_px = max(cell_im_mean(curr_mask)) * 1.5; %[HL] why *1.5? -> scale image
        ...
        if avg_active
            ...
            max_px = max(curr_act_im(repmat(curr_mask,[1,1, ...
                size(curr_act_im,3)]))) * 1.5; % fix repmat format
...
% fix size issue 
while subplots_y*subplots_x < length(rois(1).im)
    subplots_x = subplots_x+1;
end
