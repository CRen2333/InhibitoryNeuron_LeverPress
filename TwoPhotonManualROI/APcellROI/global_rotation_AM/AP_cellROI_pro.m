...
% Last Modified by GUIDE v2.5 03-Jul-2014 17:44:14
...
global_rotation = get(handles.global_rotation,'value');
...
if(global_rotation)
    disp('Correcting global rotation...');
    im_orig_avg_orig = im_orig_avg;
    [optimizer, metric] = imregconfig('monomodal');
    [im_orig_avg, R_reg] = imregister(im_orig_avg_orig, im_curr_avg, 'rigid', optimizer, metric);
    polygon_orig = polygon;
    for i=1:length(polygon.ROI)
        roi = polygon_orig.ROI{i};
        roi(:,3) = 1;
        new_roi = roi * R_reg.tdata.T;
        polygon.ROI{i} = new_roi(:,1:2);
    end
    figure;
    subplot(221);
    imshowpair(im_orig_avg_orig,im_curr_avg);
    title('green: original, purple: current');
    subplot(222);
    imshowpair(im_orig_avg,im_curr_avg);
    title('green: original (corrected), purple: current');
    subplot(223);
    imshow(im_orig_avg_orig,[]);colormap gray;hold on;
    for i=1:numel(polygon.ROI)
        plot(polygon_orig.ROI{i}(:,1),polygon_orig.ROI{i}(:,2));
    end
    title('original');
    subplot(224);
    imshow(im_orig_avg,[]);colormap gray;hold on;
    for i=1:numel(polygon.ROI)
        plot(polygon.ROI{i}(:,1),polygon.ROI{i}(:,2));
    end
...
% corrcoef_range = 0:0.05:1;
% corrcoef_bin = hist(corrcoef_grid(search_pixels_indx),corrcoef_range);

% % re-estimate the current trace
% corrcoef_threshold_grid = corrcoef_grid;
% corrcoef_threshold_grid(corrcoef_threshold_grid < corrcoef_threshold) = 0;
% corrcoef_threshold_grid = corrcoef_threshold_grid/max(corrcoef_threshold_grid(:));
% 
% updated_trace = (double(handles.im)'*corrcoef_threshold_grid(:))/sum(corrcoef_threshold_grid(:));
% 
% % Don't do this - it makes it worse for some reason?
% % update the correlation
% corrcoef_grid = zeros(N,M);
% for i = search_pixels_indx';
%     temp_corr = corrcoef(updated_trace.^3, ...
%         double(handles.im(i,:)).^3);
%     corrcoef_grid(i) = temp_corr(2);
% end

% corrcoef_threshold = mean(corrcoef_grid(search_pixels_indx)) + ...
%     2*nanstd(corrcoef_grid(search_pixels_indx));
% 
% % check for and eliminate contaminated pixels
% % get L2 norm of search area: normalize trace so sum squared = 1
% search_pixels_trace_norm = bsxfun(@minus,double(handles.im(search_pixels_indx,:)), ...
%     mean(double(handles.im(search_pixels_indx,:)),2));
% search_pixels_trace_norm = bsxfun(@times,search_pixels_trace_norm, ...
%     1./std(search_pixels_trace_norm,[],2));
% 
% % L2 normalize updated trace
% updated_trace_norm = (updated_trace-mean(updated_trace))/std(updated_trace);
% 
% diff_search_pixels = bsxfun(@minus,search_pixels_trace_norm,updated_trace_norm');


% different stragegy: check for contamination correlated with neighboring pixels

% group by k-means via correlation
% num_groups = 4;
% kmeans_idx = kmeans( ...
%     double(handles.im(search_pixels_indx,:)),num_groups,'Distance','correlation');
% temp_corr = corrcoef(double(handles.im(search_pixels_indx,:))');

%[temp kmeans_idx_sort] = sort(kmeans_idx);
%figure;imagesc(temp_corr(kmeans_idx_sort,kmeans_idx_sort));

% find the group with the highest correlation
% km = zeros(num_groups,1);
% for curr_group = 1:num_groups
%     km(curr_group) = mean(reshape(temp_corr(kmeans_idx == curr_group, ...
%         kmeans_idx == curr_group),1,[]));
% end
% 
% [km_max_mean km_idx] = max(km);
% km_max_std = std(reshape(temp_corr(kmeans_idx == km_idx,kmeans_idx == km_idx),1,[]));
% 
% corrcoef_threshold = km_max_mean-km_max_std/2;
...
% --- Executes on button press in global_rotation.
function global_rotation_Callback(hObject, eventdata, handles)
% hObject    handle to global_rotation (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of global_rotation
